...
const Card2 = ({
  ...
  return (
    ...
    >
      ...
      <CardContent className="relative z-10 flex flex-col h-full sm:p-4 md:p-6 gap-3 md:gap-5">
        {/* Header */}
        <div className="flex justify-between items-center h-8 sm:h-10 md:h-12">
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
            <div className="p-2 sm:p-2.5 md:p-3 bg-white/10 rounded-full shadow-lg backdrop-blur-lg transition-all duration-300 hover:shadow-[0_0_10px_2px_rgba(255,255,255,0.5)]">
              <MetricIcon type={type} />
            </div>
            <h2 className="text-sm sm:text-lg md:text-xl font-extrabold text-white tracking-wider uppercase">
              {title}
            </h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4">
          <div className="bg-white/10 p-1 sm:p-3 md:p-4 rounded-xl backdrop-blur-md shadow-lg flex flex-col items-center">
            {isLoading ? (
              <Loader />
            ) : (
              <>
        ...
        {/* Chart Area */}
        <div className="h-32 sm:h-40 md:h-52 w-full">{renderChart()}</div>

        {/* Duration Buttons */}
        <div className="flex justify-center gap-1 sm:gap-2 md:gap-3 mt-1 sm:mb-3 flex-wrap">
          {["today", "1M", "3M", "6M", "1Y"].map((option) => (
            <button
              key={option}
              onClick={() => handleDurationClick(option)}
              className={clsx(
                "relative z-10 px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300",
